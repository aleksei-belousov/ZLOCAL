CLASS lsc_zbp_i_online_shop_003 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
    METHODS save_modified REDEFINITION.
ENDCLASS.

CLASS lsc_zbp_i_online_shop_003 IMPLEMENTATION.

  METHOD save_modified.
    DATA : lt_online_shop_as TYPE STANDARD TABLE OF zshop_as_003,
           ls_online_shop_as TYPE                   zshop_as_003.
    IF zbp_i_online_shop_003=>cv_pr_mapped-purchaserequisition IS NOT INITIAL.
      LOOP AT zbp_i_online_shop_003=>cv_pr_mapped-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
    ...
    IF create-online_shop IS NOT INITIAL.
      ...
      INSERT zshop_as_003 FROM TABLE @lt_online_shop_as.
    ENDIF.
  ENDMETHOD.
ENDCLASS.

CLASS lhc_zbp_i_online_shop_003  DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
CLASS lhc_zbp_i_online_shop_003  IMPLEMENTATION.
...
*      SELECT SINGLE * FROM zi_online_shop_prepitem_003 WHERE Pkgid = @<key>-%param-PackageId INTO @DATA(ls_prepitem).
*
...
*      zbp_i_online_shop_003=>cv_pr_mapped = ls_pr_mapped.
    ENDLOOP.
...
* Update an existing info record
  ...
  METHOD calculate_order_id.
    DATA:
      online_shops TYPE TABLE FOR UPDATE zi_online_shop_003,
      online_shop  TYPE STRUCTURE FOR UPDATE zi_online_shop_003.

    SELECT MAX( order_id ) FROM zonlineshop_003 INTO @DATA(max_order_id).

    READ ENTITIES OF zi_online_shop_003 IN LOCAL MODE
       ENTITY Online_Shop
        ALL FIELDS
    ...
    MODIFY ENTITIES OF zi_online_shop_003 IN LOCAL MODE
    ENTITY Online_Shop UPDATE FIELDS ( CostCenter Creationdate Deliverydate Order_Id Order_Uuid Ordereditem PackageId ) WITH online_shops
    MAPPED   DATA(ls_mapped_modify)
    FAILED   DATA(lt_failed_modify)
    REPORTED DATA(lt_reported_modify).

    DATA: lt_create_pr_imp TYPE TABLE FOR ACTION IMPORT  zi_online_shop_003~create_pr,
